...
#define PUMP_MODE 0  // El modo 0 es prueba offline, el 1 es prueba online y el 2 es produccion

// -------------------------------------------------------------------------
// 0.1. MODO DE SIMULACIÓN DE SENSORES (APLICA A TODOS LOS MODOS)
// -------------------------------------------------------------------------

// Boolean que activa el modo de simulacion, pensado para probar sin sensores
#define SENSOR_SIMULATION true
...
// -------------------------------------------------------------------------
// 4. LÓGICA DE LECTURA DE HARDWARE (SOLO SI NO HAY SIMULACIÓN)
// -------------------------------------------------------------------------
...
// --- Variables y objetos para lectura de hardware real ---
    ...
    float readRealWaterLevel() {
        // Implementación real de tu sensor de nivel aquí.
        // Si usas flotadores, esta función devuelve 0% o 100% basado en el estado de los pines GPIO.
        return 72.5; // Valor estático por defecto
    }
...
void read_or_mock_sensors() {
  ...
  #else
    // CASO: Sensores Desconectados o Fallando
      current_amps = -1.0;
