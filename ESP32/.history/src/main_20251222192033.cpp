...
void publishTelemetry() {
  // 1. LEER SENSORES UNA SOLA VEZ POR CICLO (Globales)
  read_or_mock_sensors(); 
  
  // A este punto, 'current_inflow_rate' ya tiene el valor correcto 
  // (sea del sensor real o de la simulación de la calle)
  ...
  for (int i = 0; i < NUM_PUMPS; i++) {
    ...
    // 2. LECTURA ESPECÍFICA (Solo Amperaje y Temperatura son propios de la bomba)
    float pump_amps = 0.0;
    ...
    #else
      // HARDWARE REAL
      // Amperaje: Simulamos que es proporcional si está encendida (o leer pin dedicado)
      pump_amps = currentPump.is_on ? (readRealAmps() * 0.8) : 0.0; 

      // Temperatura: Leemos el sensor específico de esta bomba
      if (currentPump.id == 1) {
    ...
    // --- CORRECCIÓN AQUÍ ---
    // Usamos la variable GLOBAL que representa el flujo de entrada de la CALLE.
    // Ya no depende de si la bomba está encendida o apagada.
    doc["current_inflow_rate"] = current_inflow_rate; 
    
    doc["timestamp"] = (long)time(NULL); 
    
    // Datos compartidos (Nivel del tanque también es común)
    doc["water_level_percent"] = water_level_percent; 
    
    // Estado lógico
    doc["street_flow_status"] = (current_inflow_rate > 0) ? "FLOWING" : "STOPPED";
    ...
    // Debug
    Serial.printf("--- Bomba %d | Entrada: %.2f L/min | Estado: %s ---\n", 
      currentPump.id, current_inflow_rate, currentPump.is_on ? "ON" : "OFF");
