...
Esta guía proporciona instrucciones completas para desplegar el sistema de monitoreo y control de bombas de agua en un servidor VPS (DigitalOcean, Vultr, AWS EC2, etc.) con Debian/Ubuntu, también funciona para cualquier otro Sistema de Operación basado en Linux, adaptando el app manager (pacman, yay, etc).
...
# Instalar Git
sudo apt install -y git # En la mayoría de distros basadas en Linux viene preinstalado
...
sudo chown root passwd && sudo chgrp root passwd # Por si acaso otro usuario es el dueño y está en otro grupo

sudo chown -R postgres:postgres /var/lib/postgresql/18/main # Depende de tu versión de postgresql
...
# Hacer que docker compose esté disponible para todos los usuarios
...
- [Guía de Docker Compose YAML](https://www.geeksforgeeks.org/devops/docker-compose-yaml-explained-a-deep-dive-into-configuration/)
